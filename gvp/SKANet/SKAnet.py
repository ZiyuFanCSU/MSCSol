...
class SKABottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,
        ...
        # self.conv2 = ConvBnAct(first_planes, width, kernel_size=3,stride=stride, dilation=first_dilation, groups=cardinality, **conv_kwargs)
...
@register_model
def AFFresnext50_32x4d_zerobn(pretrained=False, **kwargs):
    model_args = dict(
        block=SKABottleneck, layers=[3, 4, 6, 3], cardinality=32, base_width=4,
        zero_init_last_bn=True, **kwargs)
    return _create_SKAresnet('SKAresnext50_32x4d', pretrained, **model_args)
