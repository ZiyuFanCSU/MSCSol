...
class GraphDataset(data.Dataset):
    ...
    def __init__(self, data_list,
        ...
        self.device = device
        self.node_counts = [len(list(e['seq'].split(" "))) for e in data_list]
    ...
    def _featurize_as_graph(self, drug):
        name = drug['name']
        id = drug['id']
        picdata = self.get_image("./dataset/image"+str(id)+".png")
        if self.normalize is not None:
            picdata = self.normalize(picdata)
        feature_molecule = drug['feature_molecule']
        with torch.no_grad():
            coords = torch.as_tensor(drug['positon'],
                                     device=self.device, dtype=torch.float32)
            seq = torch.as_tensor([self.letter_to_num[a] for a in drug['seq'].split(" ")],
                                  device=self.device, dtype=torch.long)
            solu = torch.as_tensor(drug['solution'],
                                  device=self.device, dtype=torch.float32)
        ...
        data = torch_geometric.data.Data(x=X_ca, seq=seq, name=name,solu = solu,
                                         ...
                                         edge_index=edge_index0,mask=mask,feature_molecule=feature_molecule,id = id,picdata=picdata.unsqueeze(0))
        return data
